defmodule SimpleApp.Workers.CortexLinkVerifier do
  ...
  def perform(%Oban.Job{args: args}) do
    ...
    with {:ok, response} <-
           SimpleApp.Agents.CortexLinkVerifierAgent.chat_with_default_agent(
             "Verify the links for service: #{name}",
             args
           ) do
      file_path = Path.join(["tmp", "cortex_link_verification", "#{name}.md"])
